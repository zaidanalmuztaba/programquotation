...
export function createStore({ dataDir, seedPath }) {
  ...
  function updateBackupSettings(input, actor = "Administrator") {
    const retentionDays = Number(input?.backupRetentionDays);
    if (!Number.isInteger(retentionDays) || retentionDays < 7 || retentionDays > 3650) {
      throw new Error("Retensi backup harus berupa 7-3650 hari.");
    }
    const backupMirrorPath = String(input?.backupMirrorPath || "").trim();
    if (backupMirrorPath.length > 500) {
      throw new Error("Lokasi salinan jaringan terlalu panjang.");
    }
    const updatedAt = nowIso();
    ...
    db.exec("BEGIN IMMEDIATE");
    try {
      upsert.run("backupRetentionDays", String(retentionDays), updatedAt);
      upsert.run("backupMirrorPath", backupMirrorPath, updatedAt);
      db.exec("COMMIT");
    } catch (error) {
      db.exec("ROLLBACK");
      throw error;
    }
    recordAudit(null, "BACKUP_SETTINGS_UPDATE", actor, {
      backupRetentionDays: retentionDays,
      mirrorConfigured: Boolean(backupMirrorPath),
    });
    return getSettings();
  }

  function invalidateAllSessions(actor = "System") {
    const removed = db.prepare("DELETE FROM sessions").run().changes;
    recordAudit(null, "AUTH_ALL_SESSIONS_REVOKED", actor, { removed });
    return removed;
  }
